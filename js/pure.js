// pure.js — the DOM-free core of the renderer. Every function here is a pure computation (no document,
// no window, no module-global mutable state), so it can be imported and unit-tested under Node exactly as
// the browser runs it (see bench/pure-check.js, bench/golden-render-check.js). render.js / reactive.js /
// sources.js / embed.js import from here and add only the thin DOM glue. Keeping the byte-exact hot loop,
...
export const LEVEL_CLASS = new Array(LEVELS);   // level index -> interned class-name string ("a".."h")
...
// asciiv-codec.js's buildPaletteCSS (the embed player), so the live look and the baked look never drift.
export function buildPaletteCSS(color) {
...
export function buildColorCubeCSS() {
  let css = "";
...
export function buildContrastLUT(clut, contrast, brightness, invert) {
...
export const fontPxFor = (detail) => 22 - detail * 2; // detail 1 (chunky, 20px) .. 9 (fine, 4px)
...
export function gridDims(fontPx, availW, availH, videoW, videoH) {
...
export function buildFrameHTML(data, cols, rows, s, clut, rec) {
  let out = "";
...
export const bandAvg = (a, lo, hi) => { let s = 0; for (let i = lo; i < hi; i++) s += a[i]; return s / (hi - lo) / 255; };
...
// Canonical signature that keys a baked embed. It's the source id (or file-content hash, added in embed.js)
...
export function embedSig(sourceId, s, cols, rows) {
